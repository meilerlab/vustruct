...
NGL.MenubarWidget = function (stage, preferences) {
    ...
    container.add(new NGL.MenubarVariantsWidget(stage, preferences))
    chainColorsWidget = new NGL.MenubarChainColorsWidget(stage, preferences)
    if (chainColorsWidget)
        container.add(chainColorsWidget)
...
NGL.MenubarFileWidget = function (stage) {
	...
	*/
    ...
    let createOption = UI.MenubarHelper.createOption
    let createInput = UI.MenubarHelper.createInput
    let createCheckbox = UI.MenubarHelper.createCheckbox
    let createDivider = UI.MenubarHelper.createDivider
    ...
    let menuConfig = [
        ...
        createOption('Chimera download', onChimeraOptionClick)
    ];
...
NGL.MenubarVariantsWidget = function (stage, preferences) {
    function onNeutralVariantsClick() {
        if (stage.vustruct_representations.neutralSpheres) {
            stage.vustruct_component.removeRepresentation(stage.vustruct_representations.neutralSpheres)
            stage.vustruct_representations.neutralSpheres = null;
        } else {
            stage.vustruct_representations.neutralSpheres =
                stage.vustruct_component.addRepresentation("ball+stick",
                    {
                        sele: stage.vustruct_info.neutralResidues,
                        color: "blue", radius: "0.6"
                    })
        }
    }

    function onDisease1VariantsClick() {
        if (stage.vustruct_representations.disease1Spheres) {
            stage.vustruct_component.removeRepresentation(stage.vustruct_representations.disease1Spheres)
            stage.vustruct_representations.disease1Spheres = null;
        } else {
            stage.vustruct_representations.disease1Spheres =
                stage.vustruct_component.addRepresentation("ball+stick",
                    {
                        sele: stage.vustruct_info.disease1Residues,
                        color: "red", radius: "0.6"
                    })
        }
    }

    function onDisease2VariantsClick() {
        if (stage.vustruct_representations.disease2Spheres) {
            stage.vustruct_component.removeRepresentation(stage.vustruct_representations.disease2Spheres)
            stage.vustruct_representations.disease2Spheres = null;
        } else {
            stage.vustruct_representations.disease2Spheres =
                stage.vustruct_component.addRepresentation("ball+stick",
                    {
                        sele: stage.vustruct_info.disease2Residues,
    ...
    var menuConfig = []
    var vustruct_info = stage.vustruct_info
    if (stage.vustruct_info.neutralResidueCount && vustruct_info.neutralResidues.length > 0) {
        menuConfig.push(
            UI.MenubarHelper.createOption(vustruct_info.neutralResidueCount +
                ' ' + stage.vustruct_info.neutral_variant_short_description,
                onNeutralVariantsClick));
    }
    if (vustruct_info.disease1ResidueCount && vustruct_info.disease1Residues.length > 0) {
        menuConfig.push(
            UI.MenubarHelper.createOption(vustruct_info.disease1ResidueCount +
                ' ' + stage.vustruct_info.disease1_variant_short_description,
                onDisease1VariantsClick));
    }
    if (vustruct_info.disease2ResidueCount && vustruct_info.disease2Residues.length > 0) {
        menuConfig.push(
            UI.MenubarHelper.createOption(vustruct_info.disease2ResidueCount +
                ' ' + stage.vustruct_info.disease2_variant_short_description,
                onDisease2VariantsClick));
    }


    var optionsPanel = UI.MenubarHelper.createOptionsPanel(menuConfig)

    return UI.MenubarHelper.createMenuContainer('Variants', optionsPanel)
}

NGL.MenubarChainColorsWidget = function (stage, preferences) {
    function removeChainColorRepresentations() {
        stage.vustruct_representations.chainColorsRepresentations.forEach(
            function(repr) {
            stage.vustruct_component.removeRepresentation(repr)
            }
        )
        stage.vustruct_representations.chainColorsRepresentations = []
    }

    function onColorChainsClick() {
        removeChainColorRepresentations();
        let chainColorIndex = 0;
        stage.vustruct_representations.chainColorsRepresentations =
            color_chains_consecutively(stage.vustruct_component, chainColorIndex);
    }

    function onRate4SiteColorsClick() {
        removeChainColorRepresentations();
        let schemeId = color_chains_by_rate4site_scores(
                stage.vustruct_info.rate4siteScores);
    ...
    function onDisease1PathProxColorsClick() {
        ...
        let schemeId = color_chains_by_pathprox_class(
                stage.vustruct_info.disease1PathProxScores);
    ...
    function onDisease2PathProxColorsClick() {
        removeChainColorRepresentations();
        let schemeId = color_chains_by_pathprox_class(
                stage.vustruct_info.disease2PathProxScores);

        stage.vustruct_representations.chainColorsRepresentations =
                [stage.vustruct_component.addRepresentation("cartoon", {color: schemeId})]
        }

    function onAlphafoldColorsClick() {
        removeChainColorRepresentations();
        let schemeId = color_chains_by_alpha_fold_metrics(stage.vustruct_info.alphafoldMetrics)

        stage.vustruct_representations.chainColorsRepresentations =
            [alphafoldMetricsRepresentation = stage.vustruct_component.addRepresentation("cartoon", {color: schemeId})]
        }
    ...
    let menuConfig = []
    let vustruct_info = stage.vustruct_info

    menuConfig.push(
        UI.MenubarHelper.createOption('Chains',
            onColorChainsClick));

    if (vustruct_info.rate4siteScores && Object.keys(vustruct_info.rate4siteScores)) {
        menuConfig.push(
            UI.MenubarHelper.createOption("Rate4Site scores",
                onRate4SiteColorsClick));
    }
    ...
    if (vustruct_info.disease1PathProxScores && Object.keys(vustruct_info.disease1PathProxScores)) {
        menuConfig.push(
            UI.MenubarHelper.createOption(stage.vustruct_info.disease1_variant_short_description + ' ' + 'Pathprox',
                onDisease1PathProxColorsClick));
    }

    if (vustruct_info.disease2PathProxScores && Object.keys(vustruct_info.disease2PathProxScores)) {
        menuConfig.push(
            UI.MenubarHelper.createOption(stage.vustruct_info.disease2_variant_short_description + ' ' + 'Pathprox',
                onDisease2PathProxColorsClick));
    }

    if (vustruct_info.alphafoldMetrics && Object.keys(vustruct_info.alphafoldMetrics)) {
        menuConfig.push(
            UI.MenubarHelper.createOption('Alphafold',
                onAlphafoldColorsClick));
    }

    if (menuConfig.length > 0) {
        var colorsPanel = UI.MenubarHelper.createOptionsPanel(menuConfig)

        return UI.MenubarHelper.createMenuContainer('Colors', colorsPanel)
    }
    return null;
}
...
NGL.MenubarHelpWidget = function (stage, preferences) {
    ...
    // 2022 Feb don't show on startup ever
...
NGL.OverviewWidget = function (stage, preferences) {
    ...
    /* 2022Feb Removed the show on startup box
