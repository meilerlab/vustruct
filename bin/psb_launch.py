...
def launch_one_mutation(workplan):
    ...
    if gene != geneRefseqMutation_OR_casewideString:  # If gene is not casewide run, then fill it in specifically
        geneRefseqMutation_OR_casewideString = "%s_%s_%s" % (
        gene, refseq, mutation)  # This is usual case for the gene by gene mutation run sets
    ...
    def build_slurm_dir(flavor):
        return os.path.join(mutation_dir, "slurm", ("PathProx" if "_PP_" in flavor else flavor))
    ...
    for index, row in df_all_jobs.iterrows():
        if (not df_prior_success.empty) and (row['uniquekey'] in df_prior_success.index):
            LOGGER.info("Job %s was successful previously.  Prior results will be copied to new workstatus file" % row[
                'uniquekey'])
...
""" % (slurm_file, __file__, workplan, str(datetime.datetime.now()), subdir))
                ...
                slurmDict['output'] = "%s/%s.out" % (
                slurm_stdout_directory, "%s_%%A_%%a" % geneRefseqMutation_OR_casewideString)
...
""" % (os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "psb_prep.bash"), allrows_cwd,
       allrows_cwd))
            ...
            if args.nosbatch:
                LOGGER.warning("sbatch will not be called to launch %s because args.nosbatch=%s", slurm_file,
                               args.nosbatch)
            else:
    ...
    df_running_jobs = df_all_jobs.merge(pd.DataFrame(list(jobids.items()), columns=['uniquekey', 'jobid']),
                                        on='uniquekey', how='left')
    df_running_jobs = df_running_jobs.merge(pd.DataFrame(list(arrayids.items()), columns=['uniquekey', 'arrayid']),
                                            on='uniquekey', how='left')
    df_running_jobs = df_running_jobs.merge(pd.DataFrame(list(jobinfo.items()), columns=['uniquekey', 'jobinfo']),
                                            on='uniquekey', how='left')
    df_running_jobs = df_running_jobs.merge(
        pd.DataFrame(list(jobprogress.items()), columns=['uniquekey', 'jobprogress']), on='uniquekey', how='left')
    df_running_jobs = df_running_jobs.merge(pd.DataFrame(list(exitcodes.items()), columns=['uniquekey', 'exitcodes']),
                                            on='uniquekey', how='left')
...
if oneMutationOnly:
    ...
    udn_csv_filename = os.path.join(collaboration_dir,
                                    "%s_missense.csv" % args.projectORworkplan)  # The argument is an entire project UDN124356
