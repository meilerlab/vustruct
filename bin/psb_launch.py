...
help="Create slurm files, but do NOT exec sbatch to launch them.", default=False, action = "store_true")
...
"""%(os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))),"psb_prep.bash"),allrows_cwd,allrows_cwd))
            ...
            if args.nosbatch:
                LOGGER.warning("sbatch will not be called to launch %s because args.nosbatch=%s",slurm_file,args.nosbatch)
            else:
