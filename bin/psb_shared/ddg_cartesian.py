...
class DDG_cartesian(DDG_base):
    ...
    @property
    def analyze_cartesian_ddg(self) -> Union[pd.DataFrame, None]:
        """
...
COMPLEX:   Round3: MUT_106VAL:  -615.593  fa_atr: -1141.217 fa_rep:   147.892 fa
        ...
        LOGGER.info("Parsed %d WT rows and %d MUT rows from %s", len(wt_energy_dictionaries),
                    len(mut_energy_dictionaries), self._ddg_predictions_filename)
        ...
        if abs_mut_energy_2_lowest_diff > 1:
            LOGGER.warning(
                "DDG Cartesian min energy mutant calculations did not converge to within 1 Rosetta E.U.  Difference=%f",
                abs_mut_energy_2_lowest_diff)
        else:
            LOGGER.warning("DDG Cartesian min energy mutant calculations converged to %f Rosetta Energy Units",
                           abs_mut_energy_2_lowest_diff)
    ...
    def run(self) -> Tuple[bool, pd.DataFrame]:
        ...
        def run_part1_relax() -> str:
            """
            Relax (part1) creates a number of candidate 'relaxed' .pdb files.  In part 2, we select
            lowest energy conformation for ddg cartesian calculation

            If relax is already completed for this structure, then the computation is skipped, and the target scores
            file is returned directly from the prior work.

            @return: The relative path to the target scores file, from which the lowest energy structure can be
                     further processed
            """
            ...
            def load_from_prior_relax() -> Tuple[int, str, str]:
...
endrepeat"""
                ...
                # Taken directly from 2020 Frenz et al. supplement information
...
""")
                ...
                if return_code == 0:
                    rename_succeeded = self._attempt_directory_rename_delete_if_fail(tmp_directory, relax_directory)

                if not rename_succeeded:
                    # Load their outputs so all the ddgs are on the same page
                    return_code, stdout, stderr = load_from_prior_relax()
        ...
        target_score_df = pd.read_csv(target_score_relpath, skiprows=1, sep=r"\s+")
        ...
        #############################################################################################
        def run_part2_ddg_cartesian() -> None:
            ...
            # Or somehow there without good exit code (should be impossible)
            # Then rerun
            def load_from_prior_ddg_cartesian_run() -> Tuple[int, str, str]:
                """
                @return: Return the previous exit code, as well as stdout and stderr results from previous run
                """
            ...
            if os.path.isdir(self._ddg_cartesian_final_directory):
                ...
                #     '-ddg::mut_file ' + self._mutation_list_filename, # the list of point mutations to consider
                ...
                ddg_cartesian_command.extend([
                    ...
                    "-ddg::bbnbrs 1",
                    "-score:weights ref2015_cart",
                    '-ddg::mut_file ' + self._mutation_list_filename,
                ...
                if not rename_succeeded:
                    ...
                    # Load their outputs so all the ddgs are on the same page
                    returncode, stdout, stderr = load_from_prior_ddg_cartesian_run()
                    assert returncode == 0
    ...
    def retrieve_result(self) -> Union[pd.Series, None]:
        """
        ...
        if previous_exit is None:
            ...
            LOGGER.info(
                "ddg Results exit status file %s contains non-zero exit failure: %s" % (exitcd_fullpath, previous_exit))
