...
In the VUSTruct pipeline, this is all run in the output directory for the case, 
with path components ending structure_id/PeSTo

Before running the container, VUStruct run_pesto.py will have created a PDB with
renumbered (per transcript) residues and will tabulate results on exit

This code here has NO knowledge of the outer wrapper
"""
...
parser = argparse.ArgumentParser() # psb_config.create_default_argument_parser(__doc__, os.path.dirname(os.path.dirname(__file__)))
...
# cache_dir = "/tmp/cache"
# os.makedirs("./pesto_log", exist_ok = True)
# log_filename = "./pesto_log/apply_model.log"
apply_model_log_filename = os.path.join(args.data_dir, "apply_model.log")

LOGGER.info("Log file is %s" % apply_model_log_filename)
need_roll = os.path.isfile(apply_model_log_filename)
LOGGER.info("Command: %s" % ' '.join(sys.argv))

logger_fh = RotatingFileHandler(apply_model_log_filename, backupCount=5)
...
LOGGER.info("Processing the following pdb files %s" % str(pdb_filepaths))
...
# debug print
LOGGER.info("Dataset len = %d pdb files", len(dataset))
